(function(global) {
  ...
  var _template_export_row = function(bg, body, width) {
    var item = ('<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" bg-color="'+bg+'" style="background-color:'+bg+' !important">' +
      '<tr>' +
      '  <td valign="top">' +
      '    <div style="max-width: '+width+'; margin: auto;" class="email-container">' +
      '     '+body+' ' +
      '    </div>' +
      '  </td>' +
      '</tr>' +
    ...
    jQuery(pnlAccordionItem).each(function(index) {
      ...
      (function(_element, _id) {
        ...
        utils.loadFile(_web_url + _element.attr('data-url') + '?v=' + _web_version, function(responseText) {
          ...
          __set_content(_id, responseText.split('[site-url]').join(utils.getURI()));
          _element.html(responseText.split('[site-url]').join(utils.getURI()));
        });
      }(_element, _id));
    });
  }
  ...
  var _clickEvent = function(_self) {
    ...
    if (_parentItem.hasClass(collapsedClass)) {
      ...
      _parentItem.find(content).animate({
        ...
        _parentItem.find(content).attr('style', '')
      });
...
(function(global) {
  ...
  var _tinyMceInit = function() {
    tinymce.init({
      ...
      plugins: ' table  searchreplace autolink directionality  visualblocks visualchars  image link media  codesample  charmap hr    insertdatetime advlist lists textcolor wordcount   imagetools    contextmenu colorpicker textpattern ',
      toolbar1: ' fontselect fontsizeselect  | bold italic strikethrough forecolor backcolor | link | alignleft aligncenter alignright alignjustify  | numlist bullist outdent indent  | removeformat | mybutton  ',
  ...
  utils.loadFuntions.push(_load_content_editor);
...
(function(global) {
  ...
  var _settings_image = function() {
    ...
    if (__autoWidth == 'true') {
      ...
      var __width = Math.round(__img.width() / __column.width() * 100);
      jQuery('#settings-image-width-value').text(__width + '%');
      jQuery('#settings-image-width').val(__width);
    }
  ...
  jQuery(document).on('change', '#settings-image-width', function(e) {
    ...
    __img.css('width', _self.val() + '%');
  });
  jQuery(document).on('change', '#settings-image-width-auto', function(e) {
    if (jQuery(this).is(':checked')) {
      jQuery('#settings-image-width-row').hide();
      ___get_column_element('image .img').css('width', '');
    } else {
      jQuery('#settings-image-width-row').show();
      ___get_column_element('image .img').css('width', '100%');
    }
  });
...
(function(global) {
  ...
  var __helper_export_html = function(change_shortcode) {
    ...
    __preview_content.find('.ec-preview-content-sortable-row').each(function() {
      ...
      __exported_rows += templates.export_row(__row_bg, __row_body.html(), ___width + 'px');
    });

    var __exported_template = __helper_template_export(___general_bg, __exported_rows, ___custom_css);
    return __exported_template;
  };
  var __helper_template_export = function(bg, body,custom_css) {
  ...
  var __helper_import_json_general_settings = function(__generalSettings) {
    ...
    jQuery('#settings-email-width-value').text(__generalSettings.width);
...
(function() {
  ...
  var btn_fileLabel = '.ec-modal-input';
  var activeClass = 'active';
